...
# -----------------------------------------------------------
# 🧭 PAGE NAVIGATION STATE (PERSIST AFTER RELOAD)
# -----------------------------------------------------------
if "page" not in st.session_state:
    st.session_state.page = st.query_params.get("page", ["home"])[0]
...
# -----------------------------------------------------------
    ...
    try:
        ...
        st.error("⚠️ File 'creditcard.csv' not found in your app folder.")
    st.markdown('</div>', unsafe_allow_html=True)
...
# -----------------------------------------------------------
    ...
    - **Developed by:** Netal | CSE (AIML)
    - **Tech Used:** Python, Streamlit, Scikit-learn, Pandas, NumPy
    """)
    st.markdown('</div>', unsafe_allow_html=True)
>>>>>>> e9f6cc12ecd478e0419c17a7039f4c7289774f6a
